...
/**
 * lset remove to the Aerospike Server.  Call the C Citrusleaf Client to
 * perform the write.
 *
 * Note that there are two different return codes -- one is for the gateway
 * function (GW RC), which returns "error" up to the client if something
 ...
 * and the internal "error" is passed up.
 */
GW_RC gw_lset_remove(int con_h, char *ns, char *set, cl_object *key, char* ldt, cl_object *val,
     cl_write_parameters *cl_w_p, int *cl_rc)
{
  static char * meth = "gw_lset_remove()";
  if( TRA_ENTER )
    fprintf(stderr,"%s[%s]:   \n", D_ENTER, meth  );
  ...
  // Now we're including the Write Options (Jan 2013: tjl)
  *cl_rc =
    citrusleaf_lset_remove(my_config->asc, ns, set, key, ldt, val, cl_w_p);
 ...
 * and the internal "error" is passed up.
 */
GW_RC gw_lset_size(int con_h, char *ns, char *set, cl_object *key, char* ldt,
    int *n_bins, int timeout_ms, int *cl_rc)
{
  static char * meth = "gw_lset_size()";
  if( TRA_ENTER )
    fprintf(stderr,"%s[%s]:   \n", D_ENTER, meth  );
  ...
  *cl_rc = citrusleaf_lset_size(my_config->asc, ns, set, key, ldt, n_bins,
  timeout_ms, NULL);
