 ...
 */
static ERL_NIF_TERM
util_convert_cl_bin_to_list(ErlNifEnv* envp, cl_bin *bins, int n_bins)
{
  int i;
  static char * meth = "util_convert_cl_bin_to_list()";
  if(TRA_ENTER) fprintf(stderr,"%s[%s]: NBins(%d)\n", D_ENTER, meth, n_bins);
  ...
  for (i = 0; i < n_bins; i++) { // For EACH Bin ...
    ...
    curr_head = enif_make_list_cell(envp, binval_term, curr_head);
  } // end for each BIN
 ...
 */
static ERL_NIF_TERM
lset_add7_nif(ErlNifEnv* envp, int argc, const ERL_NIF_TERM argv[])
{
  int con_h;
  char ns[NSNAME_MAX+1];
  char set[SETNAME_MAX+1];
  ...
  static char * meth = "lset_add7_nif()";
  if(TRA_ENTER) fprintf(stderr,"%s[%s]: argc(%d)\n",D_ENTER,meth,argc);
  ...
  int cl_rc;
  GW_RC gw_rc =
    gw_lset_add(con_h, ns, set, &key_obj, ldt, &value_obj, clwp_ptr, &cl_rc);
 ...
 */
static ERL_NIF_TERM
lset_scan6_nif(ErlNifEnv* envp, int argc, const ERL_NIF_TERM argv[])
{
  int con_h;
  char ns[NSNAME_MAX+1];
  char set[SETNAME_MAX+1];
  ...
  int cl_rc;
  GW_RC gw_rc =
    gw_lset_scan(con_h, ns, set, &key_obj, ldt, &bins, &n_bins, timeout_ms, &cl_rc);
