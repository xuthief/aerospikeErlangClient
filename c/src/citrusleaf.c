...
citrusleaf_lset_add(cl_cluster *asc, const char *ns, const char *set, const cl_object *key, const char *ldt, const cl_object *pval, const cl_write_parameters *cl_w_p)
{
    ...
    extern cl_rv
citrusleaf_lset_scan(cl_cluster *asc, const char *ns, const char *set, const cl_object *key, const char *ldt, cl_bin **bins, int *n_bins, int timeout_ms, uint32_t *cl_gen)
{
    if (!g_initialized) return(-1);
    ...
    {
        .free = false,
        .value = LDT_SET_OP_SCAN,
        .len = strlen(LDT_SET_OP_SCAN)
    };
    ...
    cl_bin *bins_a = NULL;
    int rc = do_the_full_monte_a( asc, 0, CL_MSG_INFO2_WRITE, 0, ns, set, NULL, &digest, 
            &bins_a, CL_OP_WRITE, 0, &n_bins_a, NULL, NULL, 
            &trid, NULL, &call, NULL);
...
size_t unpack_arg(const cl_object *arg, unsigned char* buf)
{
    size_t offset = 0;
    ...
    switch (type) {
        case 0xc0:
            citrusleaf_object_init(arg);
            break;
        case 0xd0:
            {
                int8_t v = *(int8_t*)(buf + offset);
                offset ++;
                citrusleaf_object_init_int(arg, v);
            }
            break;
        case 0xcc:
            {
        ...
        case 0xd1:
            {
                ...
                offset += 2;
                int16_t swaped = cf_swap_from_be16(v);
                citrusleaf_object_init_int(arg, swaped);
            }
            break;
        case 0xcd:
            {
                uint16_t v = *(uint16_t*)(buf + offset);
                offset += 2;
                uint16_t swaped = cf_swap_from_be16(v);
                citrusleaf_object_init_int(arg, swaped);
            }
            break;
        case 0xd2:
            {
                ...
                int32_t swaped = cf_swap_from_be32(v);
                offset += 4;
                citrusleaf_object_init_int(arg, swaped);
            }
            break;
        case 0xce:
            {
                uint32_t v = *(uint32_t*)(buf + offset);
                offset += 4;
                uint32_t swaped = cf_swap_from_be32(v);
                citrusleaf_object_init_int(arg, swaped);
            }
            break;
        case 0xd3:
            {
                uint64_t v = *(uint64_t*)(buf + offset);
                offset += 8;
                int64_t swaped = cf_swap_from_be32(v);
                citrusleaf_object_init_int(arg, swaped);
            }
            break;
        case 0xcf:
            {
                uint64_t v = *(uint64_t*)(buf + offset);
                offset += 8;
                uint64_t swaped = cf_swap_from_be32(v);
                citrusleaf_object_init_int(arg, swaped);
            }
            break;
...
size_t unpack_args(char* buf, cl_bin **bins, int *n_bins)
{
    // args cnt
    size_t offset = 0;
    uint8_t type = buf[offset++];
        ...
        default:
            if ((type & 0xf0) == 0x90) { // list with 8 bit combined header
                *n_bins = type & 0x0f;
                ...
                // no list, so n_bins to 1 and unpack whole buf
                *n_bins = 1;
                offset --;
            }
    }
    ...
    // malloc bins sz 
    *bins = (cl_bin *)malloc(sizeof(cl_bin*) * (*n_bins)); 
    for (int i=0; i<*n_bins; i++) {
        offset += unpack_arg(&(((*bins)+i)->object), buf+offset);
    }
